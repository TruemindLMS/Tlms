    ...
    return (
        <section ref={sectionRef} className="py-24 bg-gradient-to-b from-white to-gray-50">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                {/* Featured Project */}
                <div className={`mb-16 transition-all duration-700 delay-200 ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'}`}>
                        ...
                        <div className="flex gap-1">
                            {projects.map((_, i) => (
                                <button
                                    ...
                                    className={`h-1.5 rounded-full transition-all ${activeProject === i ? 'w-6 bg-emerald-600' : 'w-1.5 bg-gray-300'}`}
                                />
                            ))}
                        </div>
                    </div>
                    ...
                    <div className="grid md:grid-cols-2 gap-6">
                        <div className="bg-white rounded-2xl overflow-hidden shadow-md hover:shadow-xl transition-all">
                            <img src={projects[activeProject].image} alt="" className="w-full h-48 object-cover" />
                            <div className="p-6">
                                <div className="flex items-center gap-2 mb-2">
                                    <span className="text-xs font-medium text-emerald-600 bg-emerald-50 px-2 py-1 rounded-full">LIVE</span>
                                    <span className="text-xs text-gray-400">{projects[activeProject].team}</span>
                                </div>
                                <h4 className="text-xl font-bold text-gray-900 mb-2">{projects[activeProject].title}</h4>
                                <p className="text-gray-500 text-sm mb-4">Impact: {projects[activeProject].impact}</p>
                                <div className="mb-4">
                                    <div className="flex justify-between text-sm mb-1">
                                        <span className="text-gray-500">Progress</span>
                                        <span className="text-emerald-600 font-medium">{projects[activeProject].progress}%</span>
                                    </div>
                                    <div className="w-full bg-gray-100 rounded-full h-1.5">
                                        <div className="bg-emerald-500 rounded-full h-1.5" style={{ width: `${projects[activeProject].progress}%` }} />
                                    </div>
                                </div>
                                <button className="w-full py-2 text-sm font-medium text-emerald-600 border border-emerald-200 rounded-lg hover:bg-emerald-50 transition-all">
                                    Join Project →
                                </button>
                            </div>
                        </div>
                        ...
                        <div className="space-y-4">
                            <div className="bg-white rounded-xl p-5 shadow-sm">
                                <h4 className="font-semibold text-gray-900 mb-3">Cross-Functional Team</h4>
                                <div className="flex gap-2 mb-4">
                                    {projects[activeProject].depts.map((d, i) => (
                                        <div key={i} className="flex items-center gap-1 text-xs bg-gray-50 px-2 py-1 rounded-full">
                                            {d === "Engineering" && <Code size={12} />}
                                            {d === "Design" && <Paintbrush size={12} />}
                                            {d === "Data Science" && <Database size={12} />}
                                            <span>{d}</span>
                                        </div>
                                    ))}
                                </div>
                                <div className="flex items-center gap-2 text-sm text-gray-500">
                                    <Users size={14} />
                                    <span>{projects[activeProject].members} members • 3 departments</span>
                                </div>
                            </div>
                            <div className="bg-gradient-to-r from-emerald-50 to-blue-50 rounded-xl p-5">
                                <div className="flex items-center gap-2 mb-2">
                                    <Target size={16} className="text-emerald-600" />
                                    <h4 className="font-semibold text-gray-900">Looking for</h4>
                                </div>
                                <p className="text-sm text-gray-600">UI/UX Designer to join the team</p>
                            </div>
                        </div>
                    </div>
                </div>
